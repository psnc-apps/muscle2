...
public:
  ...
  Connection(tcp::socket * s, char * requestBuffer)
    : firstSocket(s), reqBuf(requestBuffer), closing(false), secondSocket(0), hasRemotePeer(false), referenceCount(0)
  {
  }
  
  /** Opening connection from REMOTE */
  Connection(Header h, tcp::socket * s);
