...
public class PropertiesTool {
	...
	public static boolean hasKeys(Properties props, String[] keys) {
		...
		assert props != null;
		assert keys != null;
		return hasKeys(props, new CArrayList<String>(keys));
	}
	...
	*/
	public static <T> T getPropertyWithType(Properties props, String key, Class type) throws java.lang.reflect.InvocationTargetException {
	...
	public static boolean hasKeysWithTypes(Properties props, HashMap<String, Class> types) {
		...
		for(Iterator<String> iter = types.keySet().iterator(); iter.hasNext();) {
			String key = iter.next();
			if( !hasKeyWithType(props, key, types.get(key)) )
				return false;
		}
	...
	public static boolean hasKeyWithType(Properties props, String key, Class type) {
		...
		// we use the valueOf method of the given class
		try {
			Object value = Invoker.invokeMethod(type, "valueOf", textValue);
		}
		catch (NoSuchMethodException e) {
