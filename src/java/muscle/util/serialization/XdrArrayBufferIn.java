...
public class XdrArrayBufferIn implements XdrDecodingStream {
	private final static Logger logger = Logger.getLogger(XdrArrayBufferIn.class.getName());
	private final XDRInByteArray buffer;
	 ...
	 */
	public XdrArrayBufferIn(InputStream in, int bufsize) {
		this.buffer = new XDRInByteArray(in, bufsize);
	}
	...
	private void fill(int n) throws IOException {
		if (fragmentRemaining == 0) {
			...
			buffer.read(4);
			fragmentRemaining = buffer.getInt();
	...
	public void xdrDecodeOpaque(byte[] buf, int offset, int len) throws IOException {		
		...
		while (len > 0) {
			fill(0);
			final int cp = buffer.getPart(buf, offset, Math.min(fragmentRemaining, len));
